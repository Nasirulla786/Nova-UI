...
// Variants without transition inside
const spanVariants = {
...
export function Toolbar({ className, activeColor = "text-primary" }: ToolbarProps) {
  ...
  return (
    <div className="space-y-2">
      <div
        className={cn(
          "flex items-center gap-3 p-2 relative bg-background border rounded-xl transition-all duration-200",
          className
        )}
      >
        {/* Notification */}
        ...
        <div className="flex items-center gap-2">
          {toolbarItems.map((item) => (
            <motion.button
              ...
              initial={{ gap: 0, paddingLeft: 8, paddingRight: 8 }}
              animate={{
                gap: selected === item.id ? 8 : 0,
                paddingLeft: selected === item.id ? 16 : 8,
                paddingRight: selected === item.id ? 16 : 8,
              }}
              transition={{ type: "spring", bounce: 0, duration: 0.4 }}
              className={cn(
                "relative flex items-center rounded-none px-3 py-2 text-sm font-medium transition-colors duration-300",
                selected === item.id
          ...
          <motion.button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-xl border border-primary/30 shadow-sm transition-all duration-200 hover:shadow-md hover:bg-primary/90 hover:border-primary/40 active:border-primary/50"
          >
